...
def _iter_rgb_nodes(value: Any):
    if isinstance(value, dict):
        ...
        for child in value.values():
            yield from _iter_rgb_nodes(child)
    elif isinstance(value, list):
        for child in value:
            yield from _iter_rgb_nodes(child)


def avg_rgb(value: Any) -> tuple[int, int, int] | None:
    total_r = total_g = total_b = 0
    count = 0
    for r, g, b in _iter_rgb_nodes(value):
        total_r += r
        total_g += g
        total_b += b
        count += 1
...
def avg_rgb_by_side(measured: dict[str, Any]) -> dict[str, tuple[int, int, int] | None]:
    layer1 = measured.get("layer1")
    if not isinstance(layer1, dict):
        return {}
    out: dict[str, tuple[int, int, int] | None] = {}
    for side_name, side_value in layer1.items():
        out[side_name] = avg_rgb(side_value)
    return out
...
class MyApi:
    ...
    async def get_measured(self) -> Any:
        """Fetch current Ambilight measured colors."""
        response = await self.client.get(f"{self.url}/ambilight/measured")
        await asyncio.sleep(RATE_LIMIT)
        response.raise_for_status()
        self._measured = response.json()
        return self._measured

    def get_measured_cached(self) -> dict[str, Any]:
        """Return last measured Ambilight payload."""
        if isinstance(self._measured, dict):
            return self._measured
        return {}
